import numpy as np  # Классами нейронки пишут только бабуины. Имей это в виду.
...
W2 = np.array([[0.41186367, 0.15406952, -0.47391773],
               ...
               [0.26099978, 0.12834471, 0.20866959]])  # ещё одна найденная матрица весов (уже на 2‑й слой)
b2 = np.array([-0.16286677, 0.06680119, -0.03563594])  # плюс найденные смещения (тоже на 2‑й)


# Далее прямое распространение (Inference)
# x @ W1 - умножение матрицы на вектор (лол, а что так можно было????)

def relu(t):  # он большой мальчик, может хавать вектора и не давиться
    return np.maximum(t, 0)  # максимум из каждого значения для нуля (спс numpy)
...
def predict(x):  # предсказание нейронки
    t1 = x @ W1 + b1  # линейная часть
    h1 = relu(t1)
    t2 = h1 @ W2 + b2  # перемножение матрицы и вектора
    z = softmax(t2)
    return z


probs = predict(x)
pred_class = np.argmax(probs)  # получение индекса эл-та вектора, где достигается максимум
